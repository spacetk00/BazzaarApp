import React from 'react';
import {View, Text, Image, TouchableOpacity} from 'react-native';
import {page} from '../../Styles/Style';

export default function Aramis({navigation}) {
    const prodP1 = require('../../Images/principal/aramis.jpg');
  return (
    <View>
        <View style={page.container}>
            <TouchableOpacity onPress={() => {navigation.goBack();}} style={page.botaoVoltar}><Text style={page.textoBotao}>〈 Voltar</Text></TouchableOpacity>
            <View style={page.containerImagem}>
                <Image source={prodP1} style={page.imagem}></Image>
            </View>
            <Text style={page.textoDescricao}>Camiseta Aramis Estampada</Text>
            <Text style={page.textoDescricao}>Valor: <Text style={page.textoDestaque}>R$ 84,99</Text></Text>
        </View>
    </View>
  );
}
